...
class Formdesign
{
    ...
    public function parse_table($formid, $add_fields)
    {
        ...
        $sql = "select table_name from information_schema.tables  where table_name = '" . $tname."'";
        echo $sql;
        // $sql = "select count(*) from " . $tname . " where id = '1'";
        $exist = M()->execute($sql);
        // var_dump(!$exist);
        echo "count:".count($exist);
        if (!$exist) {
            // echo "string1";
            $fields = '';
    ...
    {
        $tpl_data = $this->mb_unserialize($form['content_data'], 0, 295);
        ...
        foreach ($tpl_data as $key => $value) {
            ...
            switch ($value['leipiplugins']) {
                ...
                case 'qrcode'://未处理生成二维码
                    ...
                    if ($value['value'])
                    // dump($value);
                    $qrcode_value = $this->mb_unserialize($value['value'], 0, 395);
                ...
                case 'listctrl':
                        ...
                        //编辑数据
                    $def_value[$key] = !empty($value['value']) ? $this->mb_unserialize($value['value'], 0, 428) : '';
                    ...
                    // dump("~~title~~coltype~~unit~~sum~~colvalue~~~~~~~~~~~~~");
                    // dump($value);
                    // dump($orgtitle_arr);
                    // dump($orgcoltype_arr);
                    // dump($orgunit_arr);
                    // dump($orgsum_arr);
                    // dump($orgcolvalue_arr);
                    ...
                    } else//可写
                    {
                        ...
                        foreach ($orgtitle_arr as $k => $val) {
                           ...
                           //tbody
                            ...
                            // dump('k');
                            // print($k); 
                            // dump($orgcolvalue_arr[$k]);
                                // echo "---value:" . $value['name'];
                                // echo "--k:" . $k . "------";
                                // echo " ---orgunit_arr:" . $orgunit_arr[$k];
    ...
    {
        $tpl_data = $this->mb_unserialize($form['content_data'], 804);

        echo "form['content_data']";
        dump($form['content_data']);
        echo "tpl_data";
        dump($tpl_data);
        $return_data = array();
        foreach ($tpl_data as $key => $value) {
    ...
    function mb_unserialize($str, $mod = 0, $addres = 0)
    {
        // echo "str";
        // dump($str);
        // echo "addrea";
        ...
        switch ($mod) {
            case 1:
                $str1 = unserialize(preg_replace('!s:(\d+):"(.*?)";!se', '"s:".strlen("$2").":\"$2\";"', $str));
                break;
            case 2:
                $str1 = unserialize(preg_replace_callback('#s:(\d+):"(.*?)";#s', function ($match) {
                    return 's:' . strlen($match[2]) . ':"' . $match[2] . '";';
                }, $str));
                break;
            case 3:
                $str1 = preg_replace_callback('#s:(\d+):"(.*?)";#s', function ($match) {
                    return 's:' . strlen($match[2]) . ':"' . $match[2] . '";';
                }, $str);
                break;
            default:
